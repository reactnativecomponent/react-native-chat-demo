 ...
 * @Last Modified time: 2019-03-27 16:34:42
 */
import React from 'react'
import { View, FlatList, Image, ScrollView, Dimensions } from 'react-native'
import { Container, Icon, ListItem, Text, Body, Content } from 'native-base'
...
export default class RemoveUsers extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => ({
    ...
    headerRight: (
      <HeaderButtons color="#037aff">
        <Item title="确定" color="#037aff" onPress={navigation.getParam('handlerSubmit')} />
      </HeaderButtons>
    ),
  })
  // 构造
  ...
  _renderRow = ({ item, index }) => {
    ...
    return (
      <ListItem
        onPress={() => this._select(item, index)}
      >
        <Icon
