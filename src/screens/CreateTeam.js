 ...
 * @Last Modified time: 2019-03-27 16:24:24
 */
...
export default class CreateTeam extends React.Component {
  static navigationOptions = ({ navigation }) => ({
    ...
    headerLeft: (
      <HeaderButtons color="#037aff">
        <Item color="#037aff" title="取消" onPress={()=> navigation.pop()} />
      </HeaderButtons>
    ),
    headerRight: (
      <HeaderButtons color="#037aff">
        <Item color="#037aff" title="确定" onPress={navigation.getParam('handlerSubmit')} />
      </HeaderButtons>
    ),
  })
  // 构造
  ...
  createTeam = () => {
    ...
    if (members && members.length > 0) result.push(members[0].contactId)
    if (result.length > 0) {
      NimTeam.createTeam(
        {
          name: '群聊',
  ...
  _renderRow = ({item, section, index}) => {
    ...
    return (
      <ListItem
        onPress={() => this._select(item, section.key, index)}
      >
        <Icon
