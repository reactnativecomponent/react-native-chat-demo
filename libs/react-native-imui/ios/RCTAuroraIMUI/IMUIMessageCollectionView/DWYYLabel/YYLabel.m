...
- (void)setupYYText:(NSString *)text andUnunderlineColor:(UIColor *)underColor{
    ...
    for (NIMInputTextToken *token in tokens)
    {
        if (token.type == NIMInputTokenTypeEmoticon){
            ...
            if (image){
                ...
                NSMutableAttributedString *attachText = [NSMutableAttributedString yy_attachmentStringWithContent:imgView contentMode:UIViewContentModeCenter attachmentSize:CGSizeMake(imgHW, imgHW) alignToFont:self.font alignment:YYTextVerticalAlignmentCenter];
